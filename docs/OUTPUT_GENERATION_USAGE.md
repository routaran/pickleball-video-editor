...
The output generation module (`src/output/`) provides four main components:
...
3. **FFmpegExporter** - Renders rally cuts directly to MP4 with hardware encoding
4. **TrainingDataGenerator** - Emits the ML training-data JSON sidecar
...
**Output Files:**
1. `{video_name}.kdenlive` - Kdenlive project file
2. `{video_name}.kdenlive.ass` - ASS subtitle file (linked to project)
...
# Files created:
# ~/Videos/pickleball/rally_game.kdenlive
# ~/Videos/pickleball/rally_game.kdenlive.ass
```
...
```
{video_basename}.kdenlive
{video_basename}.kdenlive.ass
```

Example:
- Video: `/home/user/videos/tournament_final.mp4`
- Output: `tournament_final.kdenlive`, `tournament_final.kdenlive.ass`
...
**Solution**: Ensure ASS file is in the same location as the Kdenlive project and uses the correct naming convention (`{video}.kdenlive.ass`). The file uses UTF-8 encoding with BOM for proper character handling.
