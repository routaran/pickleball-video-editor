...
```bash
# When running tests
QT_QPA_PLATFORM=xcb python tests/test_player.py video.mp4
```
...
self._player = mpv.MPV(
    wid=str(wid),
    vo="gpu-next",               # preferred visual fidelity
    gpu_context="x11",           # keep embedding on X11/XWayland
    input_default_bindings=False, # Disable MPV keyboard shortcuts
    input_vo_keyboard=False,       # Let Qt handle all keyboard input
    # ... other options
)

# App runtime uses renderer fallback:
...
QT_QPA_PLATFORM=xcb python tests/test_player.py /path/to/video.mp4

# Specify custom FPS (default is 60.0)
QT_QPA_PLATFORM=xcb python tests/test_player.py /path/to/video.mp4 30.0
```
...
# Or inline
LC_NUMERIC=C QT_QPA_PLATFORM=xcb python tests/test_player.py video.mp4
```
...
**Problem**: Video doesn't appear, or MPV opens in a separate window instead of embedding.

**Cause**: `wid` embedding is X11/XWayland-oriented; native Wayland window IDs are not compatible with this integration.
...
If GPU renderer embedding still fails on your system, switch renderer mode to `x11` in app configuration.
