...
- Player 1 (optional — can be added later via the Names button)
- Player 2 (optional for Doubles, hidden for Singles)
...
### Additional Requirements (Auto-Process Mode)
2. **Court corner calibration** must be completed (auto-process mode only)

### Optional Fields
- **Player names** are optional and can be added later via the Names button
  in the main window. Empty player name inputs are filtered out automatically.

### Visual Feedback
- **Invalid fields**: Red border (`#EF5350`)
- **Valid fields**: Normal border
- **Start Editing button**: Disabled (grayed) until video path is valid (and
  court corners are set, in auto-process mode)
...
class GameConfig:
    ...
    session_state: SessionState | None = None   # loaded session for resume
    court_corners: list[list[int]] | None = None  # 4 court corners (auto-process)
    auto_mode: bool = False         # auto-process pipeline flag
```
