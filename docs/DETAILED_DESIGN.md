...
|----------|-------|-------------|
| **START_PADDING** | -0.5 seconds | Rally cut begins **0.5 seconds before** the marked start point |
| **END_PADDING** | +1.0 seconds | Rally cut ends **1 second after** the marked end point |

**Rationale:**
- **Pre-rally padding (0.5s):** Captures the server's preparation, stance, and serve motion before the ball is struck
...
- Defined as class constants `START_PADDING` and `END_PADDING` on `RallyManager` in `src/core/rally_manager.py`
- Applied inline in `RallyManager.start_rally()` and `RallyManager.end_rally()` (no separate helper methods)
...
4. System applies -0.5s padding to start time (rally cut begins 0.5 seconds before marked point)
...
│ + set_player_names(player_names: dict) -> None                │
│ + has_player_names() -> bool                                  │
│ + reset() -> None                                             │
├───────────────────────────────────────────────────────────────┤
...
│ + first_server_player_index: Optional[int]                    │
└───────────────────────────────────────────────────────────────┘
...
├───────────────────────────────────────────────────────────────┤
│ - rallies: List[Rally]                                        │
│ - _current_rally_start: Optional[int] # Frame number          │
│ - action_stack: List[Action]          # For undo              │
│ - fps: float                          # Video frame rate      │
│ # Class constants:                                            │
│ • START_PADDING = -0.5  (seconds, pre-rally)                  │
│ • END_PADDING   = +1.0  (seconds, post-rally)                 │
├───────────────────────────────────────────────────────────────┤
│ + __init__(fps: float = 60.0)                                 │
│ + start_rally(timestamp: float, score_snapshot) -> int        │
│ + end_rally(timestamp, score_at_start, winner, ...) -> Rally  │
...
│ + delete_rally(index: int, ...) -> Tuple[Rally, list[int]]    │
│ + insert_rally(index: int, ...) -> list[int]                  │
│ + undo() -> Tuple[Action, float]                              │
│ + can_undo() -> bool                                          │
...
│ + raw_start_seconds: Optional[float]  # pre-padding mark      │
│ + raw_end_seconds: Optional[float]    # pre-padding mark      │
│ + raw_start_frame: Optional[int]      # pre-padding mark      │
│ + raw_end_frame: Optional[int]        # pre-padding mark      │
│ + score_snapshot_at_start: Optional[ScoreSnapshot]            │
│ + predicted_team: Optional[int]       # ML prediction         │
...
│ + court_corners: Optional[list[list[int]]]                    │
├───────────────────────────────────────────────────────────────┤
│ + to_dict() -> dict                                           │
